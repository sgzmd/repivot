...
def process_revolut_file(file_content: bytes, person_name: str, db: Session):
    ...
    required_cols = ['Product', 'Amount', 'Fee', 'Currency', 'Description', 'Completed Date'] # 'DateDescription' -> Description logic? 
    ...
    agg_df = df.groupby(['month_year', 'Description', 'Currency'])['TotalCost'].sum().reset_index()
