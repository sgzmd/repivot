...
def create_sample_xls():
    data = {
        ...
        'Started': ['2023-01-01 10:00:00', '2023-01-02 11:00:00', '2023-01-03', '2023-02-01'],
        'Completed Date': ['2023-01-01 10:00:00', '2023-01-02 11:00:00', '2023-01-03', '2023-02-01'],
...
def test_process_revolut_file(db_session):
    ...
    s1 = db_session.query(MonthlySummary).filter_by(description='Groceries', month_year='2023-01').first()
    assert s1.total_amount == 10.50
    assert s1.person_name == person

    s2 = db_session.query(MonthlySummary).filter_by(description='Transport', month_year='2023-01').first()
    assert s2.total_amount == 5.10  # 5.00 + 0.10
    
    s3 = db_session.query(MonthlySummary).filter_by(description='Groceries', month_year='2023-02').first()
